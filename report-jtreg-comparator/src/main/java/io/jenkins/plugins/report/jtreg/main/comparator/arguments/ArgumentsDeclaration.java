...
public final class ArgumentsDeclaration {
  ...
  public static final Argument buildConfigFindArg = new Argument("--build-config-find", "A general argument for finding information in config files inside the build directory. It takes three parameters separated by colon. First the name of the config file, second the name of the config item to find (e.g. nvr) and third the query for finding the item (XPath).", " filename:whatToFind:findQuery");
  public static final Argument jobConfigFindArg = new Argument("--job-config-find", "Same as " + buildConfigFindArg.getName() + ", however it looks inside job directory instead of build.", " filename:whatToFind:findQuery");
}
